...
**判断标准**：这些字段是否**总是一起出现、一起变化**？是 → 对象，否 → 独立

```
✅ 正确：user: { name, avatar, level }  // 用户信息总是一起获取
❌ 错误：userName, userAvatar, userLevel  // 拆散了语义整体

✅ 正确：items: Array<{ title, price, image }>  // 列表项结构
❌ 错误：titles: string[], prices: number[]  // 平行数组难以维护
```
