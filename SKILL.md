...
**核心原则：区分「业务数据」和「UI 文案」**

| 类型        | 判断标准               | 处理方式                    |
| ----------- | ---------------------- | --------------------------- |
| 业务数据    | 实际使用时会从接口获取 | Props（required，无默认值） |
| UI 固定文案 | 实际使用时不会变化     | 直接硬编码，不设 Props      |

**Props 结构设计**：语义相关的字段组合为对象，重复结构设计为对象数组
...
| 追踪表所有状态为 [✓]                     | A + B    |
| 骨架关键节点均已实现（无遗漏）           | A + B    |
| 已输出接口设计，无硬编码业务内容         | A + B    |
| 重复结构（×N）使用数组 Props + 循环      | A + B    |
